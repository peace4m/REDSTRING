...
const { requireAuth, requireAdultContent } = require('../middleware/auth.middleware');
const { examineClue, interrogateSuspect, makeAccusation, getCurrentWeather, startLabTimer } = require('../game-engine/caseEngine');
const { getFromCache, setCache } = require('../services/redisService');

// ─────────────────────────────────────────────
//  LIST CASES
// ─────────────────────────────────────────────
/**
 * Returns a summarized list of published cases.
...
router.post('/sessions/:sessionId/interrogate', requireAuth, async (req, res, next) => {
    try {
        const { suspectId, presentClueId } = req.body;
        ...
        result.aiDialogue = await generateSuspectDialogue(result.aiPromptContext, result.currentStatement);
